...
/// $(function() {$('.zoom').hoverZoom({zoom:0});});   缩略图搜索图标

///////////////////文章图片自动缩小/////////////////////////////
$(function()
{
	var $article_img=$(".context img");


	function _get_image_size(filepath)
    {
        var imgsize={
        width:0,
        height:0
	...
	{
		img=_get_image_size($(this).attr('src'));
		var old_width=img.width;
		var old_height=img.height;
		var content_width=$('#body .article:first').width()-8;
		var width=(old_width>content_width)?content_width:old_width;//适用于首页
		$(this).attr({'old_width':old_width,'old_height':old_height});	//不要ie里加上height=auto, 否则高度变成1
